...
> data (version, auth, model, working directory) that makes Hanhai CLI
> debuggable and trustworthy.

## Overview

The Hanhai CLI prints a banner at startup containing a QWEN ASCII logo
...
- **White-label / third-party brand integration**: enterprises and teams
  embedding Hanhai CLI into their own products want to display their brand
  identity rather than the default "Hanhai CLI".
...
│   │                      │         │                                     │  │
│   │  ███ QWEN ASCII ███  │         │  ① Title:    >_ Hanhai CLI (vX.Y.Z)  │  │
...
- **B. Info panel** — a bordered box containing four rows. The second
  ...
  - **B①** Title: `>_ Hanhai CLI (vX.Y.Z)` — brand text + version suffix.
...
| **B①. Title — brand text** (`>_ Hanhai CLI`) | Hard-coded in `Header.tsx`          | **Replaceable**                 | Brand surface. The leading `>_` glyph is part of the existing brand; if a user wants it gone, they simply omit it from `customBannerTitle`.                                                                  |
...
<Text bold color={theme.text.accent}>
  {customBannerTitle ? customBannerTitle : '>_ Hanhai CLI'}
</Text>
…
{customBannerSubtitle ? (
