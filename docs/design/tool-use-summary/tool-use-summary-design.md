...
## 3. Hanhai CLI Implementation

### 3.1 Flow

Hanhai CLI hooks the same lifecycle point (`useGeminiStream.handleCompletedTools`) but renders on both sides of `ui.compactMode` so the feature is useful to CLI users without any SDK plumbing.
...
| Deviation                                                                | Why                                                                                                                                                                                     |
| ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Settings layer in addition to env gate                                   | Hanhai CLI renders the label in the CLI; users need a persistent switch, not a per-shell env export.                                                                                    |
...
| Dedicated `cleanSummary` post-processing                                 | Hanhai CLI supports more heterogeneous providers than CC; some models prepend `Label:` or wrap in quotes. Normalizing at the boundary keeps the UI consistent.                          |
...
| Dual render paths (full-mode inline + compact-mode header)               | Hanhai CLI's default is `ui.compactMode: false`; without the inline full-mode render, the feature would be invisible to most users.                                                     |
