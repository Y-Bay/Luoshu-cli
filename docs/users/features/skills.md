...
> Create, manage, and share Skills to extend Hanhai CLI's capabilities.

This guide shows you how to create, use, and manage Agent Skills in **Hanhai CLI**. Skills are modular capabilities that extend the model's effectiveness through organized folders containing instructions (and optionally scripts/resources).

## Prerequisites

- Hanhai CLI (recent version)
- Basic familiarity with Hanhai CLI ([Quickstart](../quickstart.md))
...
- Extend Hanhai CLI for your workflows
...
## Instructions
Provide clear, step-by-step guidance for Hanhai CLI.
...
Hanhai CLI currently validates that:
...
Hanhai CLI discovers Skills from:
...
To view available Skills, ask Hanhai CLI directly:
...
If Hanhai CLI doesn't use your Skill, check these common issues:
...
Run Hanhai CLI with debug mode to see Skill loading errors:
...
Changes take effect the next time you start Hanhai CLI. If Hanhai CLI is already running, restart it to load the updates.
