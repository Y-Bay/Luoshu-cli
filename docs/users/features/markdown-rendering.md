...
Hanhai CLI renders common Markdown structures directly in the TUI so model
...
To start Hanhai CLI in raw mode by default, set `ui.renderMode`:
...
1. If enabled and supported, Hanhai CLI asks Mermaid CLI (`mmdc`) to render the
...
3. Otherwise, Hanhai CLI falls back to a terminal wireframe or compact text
   preview.
4. If a Mermaid diagram type cannot be previewed, Hanhai CLI shows the original
...
| `QWEN_CODE_MERMAID_ALLOW_NPX=1`             | Allows Hanhai CLI to run `npx @mermaid-js/mermaid-cli` when `mmdc` is not installed. |
...
download Mermaid CLI. During streaming, Hanhai CLI shows a bounded text preview
...
Hanhai CLI supports basic inline and block LaTeX rendering in the terminal:
