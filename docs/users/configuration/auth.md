...
Hanhai CLI supports three authentication methods. Pick the one that matches how you want to run the CLI:
...
- **How it works**: on first start, Hanhai CLI opens a browser login page. After you finish, credentials are cached locally so you usually won't need to log in again.
...
- **How it works**: Subscribe to the Coding Plan with a fixed monthly fee, then configure Hanhai CLI to use the dedicated endpoint and your subscription API key.
...
Enter `qwen` in the terminal to launch Hanhai CLI, then run the `/auth` command and select **Alibaba Cloud Coding Plan**. Choose your region, then enter your `sk-sp-xxxxxxxxx` key.
...
| `security.auth.selectedType` | Tells Hanhai CLI which protocol to use on startup (e.g. `openai`, `anthropic`, `gemini`). Without this, you'd need to run `/auth` interactively. |
| `model.name`                 | The default model to activate when Hanhai CLI starts. Must match one of the `id` values in your `modelProviders`.                                |
...
The key concept is **Model Providers** (`modelProviders`): Hanhai CLI supports multiple API protocols, not just OpenAI. You configure which providers and models are available by editing `~/.qwen/settings.json`, then switch between them at runtime with the `/model` command.
...
Hanhai CLI reads API keys from environment variables (specified by `envKey` in your model config). There are multiple ways to provide them, listed below from **highest to lowest priority**:
...
Hanhai CLI auto-loads the **first** `.env` file it finds (variables are **not merged** across multiple files). Only variables not already present in `process.env` are loaded.

Search order (from the current directory, walking upward toward `/`):

1. `.qwen/.env` (preferred — keeps Hanhai CLI variables isolated from other tools)
2. `.env`
...
> [!tip]
>
> `.qwen/.env` is recommended over `.env` to avoid conflicts with other tools. Some variables (like `DEBUG` and `DEBUG_MODE`) are excluded from project-level `.env` files to avoid interfering with Hanhai CLI behavior.
...
After launching Hanhai CLI, use the `/model` command to switch between all configured models. Models are grouped by protocol:
...
| Check current authentication     | Run `/doctor` inside Hanhai CLI                                                             |
