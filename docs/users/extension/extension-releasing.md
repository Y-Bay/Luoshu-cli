...
Hanhai CLI extensions can be distributed through [GitHub Releases](https://docs.github.com/en/repositories/releasing-projects-on-github/about-releases). This provides a faster and more reliable initial installation experience for users, as it avoids the need to clone the repository.
...
To ensure Hanhai CLI can automatically find the correct release asset for each platform, you must follow this naming convention. The CLI will search for assets in the following order:
...
Here is an example of a GitHub Actions workflow that builds and releases a Hanhai CLI extension for multiple platforms:
...
You can publish Hanhai CLI extensions as scoped npm packages (e.g. `@your-org/my-extension`). This is a good fit when:
...
Your npm package must include a `qwen-extension.json` file at the package root. This is the same config file used by all Hanhai CLI extensions — the npm tarball is simply another delivery mechanism.
...
Hanhai CLI reads npm auth credentials automatically:
