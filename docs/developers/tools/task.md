...
This document describes the `task` tool for Hanhai CLI.
...
## How to use `task` with Hanhai CLI
...
You can view available subagents by using the `/agents` command in Hanhai CLI.
...
Subagents are configured through Hanhai CLI's agent configuration system. Use the `/agents` command to:
