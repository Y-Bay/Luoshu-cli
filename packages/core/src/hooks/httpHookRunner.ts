...
/**
 * Maximum output length (10,000 characters as per Hanhai CLI spec)
 */
...
export class HttpHookRunner {
  ...
  async execute(
    ...
    try {
      ...
      try {
        ...
        // Per Hanhai CLI spec: Non-2xx status is a non-blocking error
        ...
        if (
          ...
          // Timeout or abort is a non-blocking error per Hanhai CLI spec
          debugLogger.warn(
        ...
        // Connection failure is a non-blocking error per Hanhai CLI spec
        debugLogger.warn(
   ...
   * Per Hanhai CLI spec: output is capped at 10,000 characters
   */
  ...
  private normalizeOutput(
    ...
    if ('systemMessage' in json && typeof json['systemMessage'] === 'string') {
      // Apply output length limit per Hanhai CLI spec
