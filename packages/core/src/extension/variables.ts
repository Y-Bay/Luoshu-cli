...
export const EXTENSIONS_DIRECTORY_NAME = path.join(HANHAI_DIR, 'extensions');
  ...
  try {
    ...
    for (const file of mdFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".hanhai" in markdown files
  ...
  try {
    ...
    for (const file of scriptFiles) {
      ...
      try {
        ...
        // Replace references to ".claude" directory with ".hanhai" in shell scripts
        // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
        // Avoid matching URLs, comments, or string literals containing .claude
