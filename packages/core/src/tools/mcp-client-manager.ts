 ...
 *
 * `HANHAI_SERVE_MCP_CLIENT_BUDGET` — positive integer; non-numeric /
...
function readBudgetFromEnv(): McpBudgetConfig {
  const rawBudget = process.env['HANHAI_SERVE_MCP_CLIENT_BUDGET'];
  ...
  if (rawBudget !== undefined && rawBudget !== '') {
    ...
    if (Number.isFinite(parsed) && Number.isInteger(parsed) && parsed > 0) {
      ...
      // like `HANHAI_SERVE_MCP_CLIENT_BUDGET=abc` previously fell
