...
/**
 * Per-process cache for `.hanhaiignore` discovery. The same directories show
 ...
 * `qwenIgnore`: dir → string | null (cached `.hanhaiignore` path or null)
 *
 * **Known staleness window:** a `.hanhaiignore` created mid-session, or a
...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    // Add file exclusions from .gitignore and .hanhaiignore
    ...
    if (filteringOptions.respectQwenIgnore) {
      // Load .hanhaiignore from each workspace directory, not just the primary one
      ...
      for (const searchPath of paths) {
        ...
        if (qwenIgnorePath === undefined) {
          const candidate = path.join(dir, '.hanhaiignore');
