...
const enterWorktreeDescription = `Creates an isolated git worktree at \`<projectRoot>/.hanhai/worktrees/<slug>\` and returns its absolute path so subsequent file edits, shell commands, and other tools can operate inside it.
...
> {
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    ...
    // already inside `.hanhai/worktrees/<slug>/`, a fresh worktree would
    // be provisioned at `<repo>/.hanhai/worktrees/<new>/` — but the
    ...
    // monorepo subdirectory would scatter `.hanhai/worktrees/` under each
    ...
    // `<repoRoot>/.hanhai/worktrees/`, regardless of which subdirectory
