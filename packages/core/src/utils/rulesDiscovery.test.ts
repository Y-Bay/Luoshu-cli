...
import { HANHAI_DIR, unescapePath } from './paths.js';
...
`;
  ...
  describe('loadRules', () => {
    ...
    it('loads baseline rules into content', async () => {
      const rulesDir = path.join(projectRoot, HANHAI_DIR, 'rules');
...
Use hooks.`,
    ...
    it('skips project rules when folder is untrusted', async () => {
      await createTestFile(
        path.join(projectRoot, HANHAI_DIR, 'rules', 'r.md'),
        'Untrusted.',
      );
    ...
    it('loads global rules even when folder is untrusted', async () => {
      await createTestFile(
        path.join(homedir, HANHAI_DIR, 'rules', 'g.md'),
        'Global.',
      );
      const result = await loadRules(projectRoot, false);
      expect(result.ruleCount).toBe(1);
    ...
    it('does not duplicate rules when projectRoot equals homedir', async () => {
      await createTestFile(
        path.join(homedir, HANHAI_DIR, 'rules', 's.md'),
        'Shared.',
      );
    ...
    it('formats rules with source markers', async () => {
      await createTestFile(
        path.join(projectRoot, HANHAI_DIR, 'rules', 'test.md'),
        'Content.',
      );
      const result = await loadRules(projectRoot, true);
      expect(result.content).toContain(
        `--- Rule from: ${HANHAI_DIR}/rules/test.md ---`,
      );
    });

    it('reads global rules from HANHAI_HOME when set', async () => {
      const customQwenHome = path.join(testRootDir, 'custom-qwen-home');
      const originalQwenHome = process.env['HANHAI_HOME'];
      process.env['HANHAI_HOME'] = customQwenHome;
      try {
        await createTestFile(
          path.join(customQwenHome, 'rules', 'fromCustomHome.md'),
          'CustomHome rule.',
        );
        // A stale rule in the legacy ~/.hanhai/rules location should NOT be
        // loaded once HANHAI_HOME points elsewhere.
        await createTestFile(
          path.join(homedir, HANHAI_DIR, 'rules', 'fromLegacyHome.md'),
          'LegacyHome rule.',
        );
