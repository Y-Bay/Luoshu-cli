    ...
    if (isHomeDirectory) {
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (
          ...
          currentDir === path.join(resolvedHome, HANHAI_DIR)
        ) {
          break;
        }
 ...
 * Also loads path-based context rules from `.hanhai/rules/` directories.
...
export async function loadServerHierarchicalMemory(
  ...
  // Load path-based context rules from .hanhai/rules/ directories
