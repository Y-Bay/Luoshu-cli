...
import { getProjectHash, HANHAI_DIR, sanitizeCwd } from '../utils/paths.js';
import { FatalConfigError } from '../utils/errors.js';

export { HANHAI_DIR } from '../utils/paths.js';
export const GOOGLE_ACCOUNTS_FILENAME = 'google_accounts.json';
export const OAUTH_FILE = 'oauth_creds.json';
export const SKILL_PROVIDER_CONFIG_DIRS = ['.hanhai', '.agents'];
...
export class Storage {
   ...
   *              Pass the project root so that relative values like ".hanhai" resolve
   ...
   *
   * Priority: HANHAI_RUNTIME_DIR env var > setRuntimeBaseDir() value > getGlobalQwenDir()
   * @returns Absolute path to the runtime output base directory
   */
  static getRuntimeBaseDir(): string {
    const envDir = process.env['HANHAI_RUNTIME_DIR'];
    if (envDir) {
      return (
  ...
  static getGlobalQwenDir(): string {
    ...
    if (!homeDir) {
      return path.join(os.tmpdir(), '.hanhai');
    }
    return path.join(homeDir, HANHAI_DIR);
  }
  ...
  getQwenDir(): string {
    return path.join(this.targetDir, HANHAI_DIR);
  }
  ...
  /**
   * Returns the user-level extensions directory (~/.hanhai/extensions/).
   * Extensions installed at user scope are stored here, as opposed to
   * project-level extensions which live in <project>/.hanhai/extensions/.
   */
