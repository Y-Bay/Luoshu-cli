 ...
 * `.hanhai/worktrees/.gitignore`) so it cannot leak into commits.
 */
 ...
 * Changing this value affects the on-disk layout (`~/.hanhai/<WORKTREES_DIR>/`)
...
export class GitWorktreeService {
   ...
   * scatter `.hanhai/worktrees/` under each subdirectory instead of
  ...
  // `<projectRoot>/.hanhai/worktrees/<slug>` rather than under the
  // session-scoped Arena baseDir.
  // ──────────────────────────────────────────────────────────────────────

  /**
   * Returns the directory holding all general-purpose worktrees for this
   * repo: `<projectRoot>/.hanhai/worktrees`.
   */
  getUserWorktreesDir(): string {
    return path.join(this.sourceRepoPath, '.hanhai', WORKTREES_DIR);
  }
  ...
  /**
   * Creates a general-purpose worktree at `<projectRoot>/.hanhai/worktrees/<slug>`
  ...
  async createUserWorktree(
    ...
    try {
      ...
      // touches an existing user-managed `.hanhai/.gitignore`.
  ...
  /**
   * Ensures `<projectRoot>/.hanhai/.gitignore` ignores the worktrees
  ...
  private async ensureWorktreesGitignored(): Promise<void> {
    try {
      const qwenDir = path.join(this.sourceRepoPath, '.hanhai');
