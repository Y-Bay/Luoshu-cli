...
export const AppContainer = (props: AppContainerProps) => {
   ...
   *
   * In legacy blocking mode (`HANHAI_LEGACY_MCP_BLOCKING=1`) MCP
  ...
  useEffect(() => {
    ...
    // `debugLogger.warn` so it shows up under `HANHAI_DEBUG=1` and in
    ...
    async (choice: 'keep' | 'remove' | 'cancel') => {
      ...
      if (choice === 'remove' && activeWorktree) {
        try {
          ...
          // subdir but the worktree lives at `<repoRoot>/.hanhai/worktrees/`,
  ...
  const performMemoryRefresh = useCallback(async () => {
    historyManager.addItem(
      {
        type: MessageType.INFO,
        text: 'Refreshing hierarchical memory (HANHAI.md or other context files)...',
      },
      Date.now(),
    );
    try {
  ...
  // Update terminal title with Hanhai CLI status and thoughts
  useEffect(() => {
    ...
    // Note: We don't need to reset the window title on exit because Hanhai CLI is already doing that elsewhere
  }, [
    streamingState,
    thought,
    settings.merged.ui?.showStatusInTitle,
