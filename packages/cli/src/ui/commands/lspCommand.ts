  ...
  action: async (
    ...
    if (!config.isLspEnabled()) {
      return emitMessage(
        ...
        'info',
        t(
          'LSP is not enabled. Start Hanhai CLI with `--experimental-lsp` to enable LSP support.',
        ),
      );
    }
    ...
    if (!client) {
      return emitMessage(
        ...
        'info',
        t(
          'LSP is enabled but no client is connected. Check debug logs under `${HANHAI_RUNTIME_DIR:-~/.hanhai}/debug/` or see the LSP troubleshooting docs.',
        ),
      );
    }
