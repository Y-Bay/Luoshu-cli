...
export function detectTerminalImageProtocol(
  ...
  if (env['HANHAI_DISABLE_MERMAID_IMAGES'] === '1') {
    return null;
  }

  const forced = env['HANHAI_MERMAID_IMAGE_PROTOCOL']?.toLowerCase();
...
function isMermaidImageRenderingDisabled(env: NodeJS.ProcessEnv): boolean {
  return env['HANHAI_DISABLE_MERMAID_IMAGES'] === '1';
}
...
function unavailableImageRenderingDisabled(): MermaidImageUnavailableResult {
  return {
    kind: 'unavailable',
    reason:
      'Mermaid image rendering is disabled via HANHAI_DISABLE_MERMAID_IMAGES.',
  };
}
...
export function renderMermaidImageSync({
  ...
  const imageRendering = env['HANHAI_MERMAID_IMAGE_RENDERING'];
  if (
    imageRendering !== '1' &&
    ...
    return {
      kind: 'unavailable',
      reason:
        'Mermaid image rendering is disabled by default. Set HANHAI_MERMAID_IMAGE_RENDERING=1 to enable external renderers.',
      showReason: false,
    };
  }
  ...
  if (!mmdc) {
    return {
      kind: 'unavailable',
      reason:
        'Mermaid CLI (mmdc) was not found. Install @mermaid-js/mermaid-cli, set HANHAI_MERMAID_MMD_CLI, or set HANHAI_MERMAID_ALLOW_NPX=1.',
    };
  }
...
function findMmdc(env: NodeJS.ProcessEnv): string | null {
  const explicit = env['HANHAI_MERMAID_MMD_CLI'];
  if (explicit && isExecutable(explicit)) return explicit;

  const mmdc = findExecutable('mmdc', env);
  if (mmdc) return mmdc;

  if (env['HANHAI_MERMAID_ALLOW_NPX'] === '1' && findExecutable('npx', env)) {
...
function findExecutable(
  ...
  const allowLocalRenderers =
    env['HANHAI_MERMAID_ALLOW_LOCAL_RENDERERS'] === '1';
...
function getMermaidRenderWidth(env: NodeJS.ProcessEnv): number {
  const configuredWidth = Number(env['HANHAI_MERMAID_RENDER_WIDTH']);
...
function getMermaidRenderTimeout(env: NodeJS.ProcessEnv): number {
  const configuredTimeout = Number(env['HANHAI_MERMAID_RENDER_TIMEOUT_MS']);
...
function getMermaidCellAspectRatio(env: NodeJS.ProcessEnv): number {
  const configuredAspectRatio = Number(env['HANHAI_MERMAID_CELL_ASPECT_RATIO']);
