...
describe('loadCliConfig model selection', () => {
  ...
  it.skip('uses the default gemini model if nothing is set', async () => {
    ...
    expect(config.getModel()).toBe(DEFAULT_HANHAI_MODEL);
  });
...
describe('sandbox image resolution precedence', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(os.homedir).mockReturnValue('/mock/home/user');
    vi.stubEnv('GEMINI_API_KEY', 'test-api-key');
    delete process.env['HANHAI_SANDBOX_IMAGE'];
  });
  ...
  it('uses --sandbox-image over env and settings', async () => {
    vi.stubEnv('HANHAI_SANDBOX_IMAGE', 'env-image');
    process.argv = [
      'node',
      'script.js',
      '--sandbox',
  ...
  it('uses HANHAI_SANDBOX_IMAGE over tools.sandboxImage', async () => {
    vi.stubEnv('HANHAI_SANDBOX_IMAGE', 'env-image');
...
describe('loadCliConfig runtimeOutputDir', () => {
  const originalArgv = process.argv;
  const originalRuntimeEnv = process.env['HANHAI_RUNTIME_DIR'];
  ...
  afterEach(() => {
    ...
    if (originalRuntimeEnv !== undefined) {
      process.env['HANHAI_RUNTIME_DIR'] = originalRuntimeEnv;
    } else {
      delete process.env['HANHAI_RUNTIME_DIR'];
    }
    vi.unstubAllEnvs();
  ...
  it('should resolve relative runtimeOutputDir against cwd', async () => {
    const argv = await parseArguments();
    const settings: Settings = {
      advanced: { runtimeOutputDir: '.hanhai' },
    };
  ...
  it('should let HANHAI_RUNTIME_DIR env var take priority over settings', async () => {
