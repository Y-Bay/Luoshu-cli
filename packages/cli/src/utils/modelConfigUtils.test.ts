...
describe('modelConfigUtils', () => {
  ...
  describe('resolveCliGenerationConfig', () => {
    ...
    beforeEach(() => {
      ...
      delete process.env['HANHAI_MODEL'];
    ...
    // Edge Case 2: HANHAI_MODEL is used as final fallback when OPENAI_MODEL is not set
    it('Edge Case 2: HANHAI_MODEL should be used as fallback when OPENAI_MODEL is not set', () => {
      const argv = {};
      ...
      resolveCliGenerationConfig({
        ...
        settings,
        selectedAuthType,
        env: { HANHAI_MODEL: 'qwen-env-model' },
      });
    ...
    // Edge Case 3: OPENAI_MODEL over HANHAI_MODEL when both are set and settings.model.name is not set
    it('Edge Case 3: OPENAI_MODEL should win over HANHAI_MODEL when both set', () => {
      const argv = {};
    ...
    describe('[Regression] model precedence', () => {
      ...
      it('[Regression] HANHAI_MODEL as fallback when OPENAI_MODEL not set', () => {
        const settings = makeMockSettings({ model: { name: undefined } });
        const selectedAuthType = AuthType.USE_OPENAI;
        const env = { HANHAI_MODEL: 'qwen-model', OPENAI_API_KEY: 'key' };

        vi.mocked(resolveModelConfig).mockImplementation(() => ({
          config: { model: 'qwen-model', apiKey: 'key', baseUrl: '' },
          sources: { model: { kind: 'env' as const, envKey: 'HANHAI_MODEL' } },
          warnings: [],
        }));
        ...
        // HANHAI_MODEL should be passed to resolveModelConfig
        const callArgs = vi.mocked(resolveModelConfig).mock.calls[0][0];
        expect(callArgs.env?.['HANHAI_MODEL']).toBe('qwen-model');
      });
