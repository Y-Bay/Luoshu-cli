 ...
 *
 * Activated by setting HANHAI_PROFILE_STARTUP=1. When enabled, collects
 * high-resolution timestamps at key phases of CLI startup and writes a JSON
 * report to ~/.hanhai/startup-perf/ on finalization.
 *
 ...
 * reports. Set HANHAI_PROFILE_STARTUP_OUTER=1 to also profile the outer
...
export function initStartupProfiler(): void {
  ...
  if (process.env['HANHAI_PROFILE_STARTUP'] !== '1') {
    return;
  }

  const inSandboxChild = !!process.env['SANDBOX'];
  const outerOptIn = process.env['HANHAI_PROFILE_STARTUP_OUTER'] === '1';
  ...
  // Disable with HANHAI_PROFILE_STARTUP_NO_HEAP=1 when measuring the
  // Heisenberg overhead of the heap call itself.
  captureHeap = process.env['HANHAI_PROFILE_STARTUP_NO_HEAP'] !== '1';
  finalized = false;
...
export function finalizeStartupProfile(sessionId?: string): void {
  ...
  try {
    const dir = path.join(os.homedir(), '.hanhai', 'startup-perf');
