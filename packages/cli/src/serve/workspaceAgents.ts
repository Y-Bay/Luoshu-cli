...
export function mountWorkspaceAgentsRoutes(
  ...
  app.get('/workspace/agents', async (_req, res) => {
    try {
      // `force: true` re-walks `.hanhai/agents/` on every call so out-of-
      ...
      try {
        ...
        if (err instanceof SubagentError) {
          ...
          if (err.code === SubagentErrorCode.FILE_ERROR) {
            ...
            // '/Users/<x>/.hanhai/agents/foo.md'"` — leaking the
...
function parseAgentConfig(
  ...
  // and write a project-level file at `<workspace>/.hanhai/agents/
