...
export const defaultSpawnChannelFactory: ChannelFactory = async (
  ...
  // we MUST scrub is `HANHAI_SERVER_TOKEN` (daemon-only auth that
  ...
  // `HANHAI_SERVER_TOKEN` in overrides — defense in depth).
 ...
 *
 * Currently just `HANHAI_SERVER_TOKEN`: the daemon's own bearer token,
