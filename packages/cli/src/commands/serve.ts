...
export const serveCommand: CommandModule<unknown, ServeArgs> = {
  command: 'serve',
  describe:
    'Run Hanhai CLI as a local HTTP daemon (Stage 1 experimental: --http-bridge)',
  builder: (yargs: Argv) =>
    yargs
      .option('port', {
        type: 'number',
      ...
      .option('token', {
        type: 'string',
        description:
          'Bearer token required on every request. Falls back to the HANHAI_SERVER_TOKEN env var.',
      })
      .option('max-sessions', {
        type: 'number',
        default: 20,
      ...
      .option('require-auth', {
        ...
        description:
          ...
          '127.0.0.1. Requires --token or HANHAI_SERVER_TOKEN. /health also ' +
