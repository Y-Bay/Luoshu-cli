...
A minimum experimental TypeScript SDK for programmatic access to Hanhai CLI.
...
Creates a new query session with the Hanhai CLI.
...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'qwen-max'`, `'qwen-plus'`, `'qwen-turbo'`). Takes precedence over `OPENAI_MODEL` and `HANHAI_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                             |
| `pathToQwenExecutable`   | `string`                                       | Auto-detected    | Path to the Hanhai CLI executable. Supports multiple formats: `'qwen'` (native binary from PATH), `'/path/to/qwen'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `HANHAI_CLI_PATH` env var, `~/.volta/bin/qwen`, `~/.npm-global/bin/qwen`, `/usr/local/bin/qwen`, `~/.local/bin/qwen`, `~/node_modules/.bin/qwen`, `~/.yarn/bin/qwen`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the Hanhai CLI process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                               |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in Hanhai CLI system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                               |
...
const daemon = new DaemonClient({
  ...
  token: process.env['HANHAI_SERVER_TOKEN'],
});
...
#### Hanhai CLI Installation Required

Version 0.1.0 requires [Hanhai CLI](https://github.com/QwenLM/qwen-code) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install Hanhai CLI globally
npm install -g qwen-code@^0.4.0
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate Hanhai CLI installation is needed.
