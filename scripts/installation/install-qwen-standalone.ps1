# Hanhai CLI Windows hosted PowerShell entrypoint.
...
function Install-CurrentCmdPathShim {
    ...
    foreach ($candidate in $candidates) {
        ...
        $shim = "@echo off`r`nREM Hanhai CLI current-session shim. Generated by install-qwen-standalone.ps1.`r`ncall `"$QwenCommand`" %*`r`n"
...
try {
    ...
    Write-Error "Failed to download Hanhai CLI installer from ${qwenInstallerUrl}: $($_.Exception.Message)"
    exit 1
}
...
try {
    ...
    Write-Error "Failed to download Hanhai CLI installer checksums from ${qwenChecksumsUrl}: $($_.Exception.Message)"
    exit 1
}
